...
pattern_match = re.compile("\x0f\x87....\x48\x8b\x43\x10", re.DOTALL)
    ...
    for p in poss:
        ...
        while data[new_p:new_p+2] != "\x48\x81" and data[new_p:new_p+3] != "\x48\x83\x78":
            ...
            if data[new_p] == "\x40":
                ...
                if data[new_p+1] == "\x8b" or data[new_p+1:new_p+3] == "\x89\x70" or data[new_p+1:new_p+3] == "\x8d\x78":
                    new_p += 4
                elif data[new_p+1:new_p+3] == "\x89\xea":
                    new_p += 3
                elif data[new_p+1:new_p+3] == "\x89\x44":
                    new_p += 5
                    ...
                    c += "+" + str(ord(data[new_p+3:new_p+4])) + " "
                    new_p += 4
                elif data[new_p+1:new_p+3] == "\x8d\x71" or data[new_p+1:new_p+3] == "\x8d\x72": #add/sub
                    x = ord(data[new_p+3:new_p+4])
                    if x & 0x80:
                        x = 0x100 - x
                        c += "-" + str(x) + " "
                    else:
